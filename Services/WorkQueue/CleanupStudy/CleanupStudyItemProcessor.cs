...
namespace MatrixPACS.ImageServer.Services.WorkQueue.CleanupStudy
{
    /// <summary>
            ...
            foreach (WorkQueueUid sop in WorkQueueUidList)
            {
                ...
                using (ServerCommandProcessor processor = new ServerCommandProcessor(String.Format("Deleting {0}", sop.SopInstanceUid)))
                {
                    ...
                    try
                    {
                        ...
                        MatrixPACS.ImageServer.Core.Command.DeleteDirectoryCommand deleteDir = new MatrixPACS.ImageServer.Core.Command.DeleteDirectoryCommand(fileInfo.Directory.FullName, false, true);
                ...
                if (Directory.Exists(baseDir))
                {
                    using (ServerCommandProcessor processor = new ServerCommandProcessor(String.Format("Deleting {0}", sop.SopInstanceUid)))
                    {
                        MatrixPACS.ImageServer.Core.Command.DeleteDirectoryCommand deleteDir = new MatrixPACS.ImageServer.Core.Command.DeleteDirectoryCommand(baseDir, false, true);
                        processor.AddCommand(deleteDir);
