...
namespace MatrixPACS.ImageServer.Web.Application.Pages.Admin.Configure.ServerPartitions
{
    public partial class DataAccessGroupPanel : System.Web.UI.UserControl
    {
        ...
        private void InitDataAccessGroupList()
        {
            ...
            if (Thread.CurrentPrincipal.IsInRole(MatrixPACS.Enterprise.Common.AuthorityTokens.Admin.Security.AuthorityGroup))
            {
        ...
        private DataAccessGroupInfoCollection LoadDataAccessGroupInfo()
        {
            using (AuthorityManagement service = new AuthorityManagement())
            {
                ...
                var accessToAllPartitionGroups = CollectionUtils.Select(service.ListAllAuthorityGroupDetails(),
                            g => HasToken(g.AuthorityTokens, MatrixPACS.Enterprise.Common.AuthorityTokens.DataAccess.AllPartitions));
                ...
                var list = new DataAccessGroupInfoCollection(CollectionUtils.Map<AuthorityGroupDetail, DataAccessGroupInfo>(combinedGroups,
                    (group) =>
                    {
                        ...
                        var fullServerPartitionAccess = HasToken(group.AuthorityTokens, MatrixPACS.Enterprise.Common.AuthorityTokens.DataAccess.AllPartitions);
                        var allStudiesAccess = HasToken(group.AuthorityTokens, MatrixPACS.Enterprise.Common.AuthorityTokens.DataAccess.AllStudies);
