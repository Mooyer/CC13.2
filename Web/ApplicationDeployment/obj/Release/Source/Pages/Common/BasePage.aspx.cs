...
namespace MatrixPACS.ImageServer.Web.Application.Pages.Common
{
    /// <summary>
            ...
            lock (Extensions)
            {
                if (!_extensionLoaded)
                {
                    try
                    {
                        ...
                        var attrs = MatrixPACS.Common.Utilities.AttributeUtils.GetAttributes<ExtensibleAttribute>(GetType(), true);
