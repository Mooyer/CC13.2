...
--%>
<%@ Import Namespace="MatrixPACS.ImageServer.Web.Common.WebControls.UI" %>
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SearchPanel.ascx.cs"
	Inherits="MatrixPACS.ImageServer.Web.Application.Pages.Queues.WorkQueue.SearchPanel" %>
									...
									<ContentTemplate>
										<asp:Panel ID="ToolbarButtons" runat="server" CssClass="ToolbarButtons" Style="position: relative;">
											<ccUI:ToolbarButton ID="ViewItemDetailsButton" runat="server" SkinID="<%$Image:ViewDetailsButton%>"
												Roles='<%= MatrixPACS.ImageServer.Common.Authentication.AuthorityTokens.WorkQueue.View %>' />
											<ccUI:ToolbarButton ID="RescheduleItemButton" runat="server" SkinID="<%$Image:RescheduleButton%>"
												OnClick="RescheduleItemButton_Click" Roles='<%= MatrixPACS.ImageServer.Common.Authentication.AuthorityTokens.WorkQueue.Reschedule %>' />
											<ccUI:ToolbarButton ID="ResetItemButton" runat="server" SkinID="<%$Image:ResetButton%>"
												OnClick="ResetItemButton_Click" Roles='<%= MatrixPACS.ImageServer.Common.Authentication.AuthorityTokens.WorkQueue.Reset %>' />
											<ccUI:ToolbarButton ID="DeleteItemButton" runat="server" SkinID="<%$Image:DeleteButton%>"
												OnClick="DeleteItemButton_Click" Roles='<%= MatrixPACS.ImageServer.Common.Authentication.AuthorityTokens.WorkQueue.Delete %>' />
											<ccUI:ToolbarButton ID="ReprocessItemButton" runat="server" SkinID="<%$Image:ReprocessButton%>"
												OnClick="ReprocessItemButton_Click" Roles='<%= MatrixPACS.ImageServer.Common.Authentication.AuthorityTokens.WorkQueue.Reprocess %>' />
										</asp:Panel>
									</ContentTemplate>
								</asp:UpdatePanel>
							</td>
